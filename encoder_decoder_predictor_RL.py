...
#print(type(session.run(test)))
#test.eval(session=session)
...
#def getGradientFromInitialInputVector(concat_output, model):
    #with tf.GradientTape() as tape:
        #grads_value = tape.gradient(model.get_layer('dense_8').output), model.get_layer('concatenate').output)[0]
        #print (grads_value)
        #return grads_value
...
def returnStepValues(action, alpha, concat_output, steps_per_episode, model, done):
    #next_state = state + alpha * action
    #sess = tf.InteractiveSession()
    #with sess.as_default():
        #sess.run(action)
        #act_value = action.eval()
    #step = act_value * alpha
    # Split the concat_output for x,y and the one hot vector
    #global sess
    #act = sess.run(action)
    #x = tf.keras.backend.get_value(action)
    step = action * alpha
    
    arrayChunk = np.split(concat_output[0][0],[1,2])
    ...
    #next_state_concat_output = np.append(next_state,oneHot)
    ...
    #sess = tf.compat.v1.Session()
    #Combined_arr = sess.run(Combined_arr_T)
    ...
    return Combined_arr, reward, done, steps_per_episode
...
class Critic(tf.keras.Model):
    def __init__(self):
        ...
        self.critic_output = Dense(1, activation='relu', name='critic_output')
...
class Agent():
  ...
  def __init__(self):
    ...
    self.memory = RBuffer(1_00_000,  observation_space_shape, 2)
    self.trainstep = 0
    self.replace = 5
  ...
  def update_target(self, tau=None):
      ...
      self.actor_target.set_weights(targets1)
  ...
  def train(self):
      ...
      self.c_opt.apply_gradients(zip(grads1, self.critic_main.trainable_variables))
...
agent = Agent()
#episods = 100  
# Ask Huber how many episodes start with 500
episods = 2000
ep_reward = []
total_avgr = []
...
for s in range(episods):
    ...
    concat_output = getConcatinateLayerOutput(test_nn, model)
    #it returns shape:TensorShape([None, 12])
    ...
    while not done: # cretae trejectories 
      ...
      next_state, reward, done, steps_per_episode  = returnStepValues(action, alpha, concat_output, steps_per_episode, model, done)   
      # PRINT - Reward 
