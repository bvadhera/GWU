...
#def getAccuracyLayerOutput(concat_output, model):
    #preds = accuracy_model.predict(concat_output)
    #print (preds[0])
    #return preds[0]  
...
writeFile = current_dir + "results_" + "testDataNetwork_OneHot.csv"
...
decoded_layer = Dense(int(3), activation=tf.nn.leaky_relu) 
...
history = model.fit(X_train, {'network_with_accuracy': y_train[['test_acc', 'num_layers_N', 'num_node1_N', 'num_node2_N']] }, validation_split=0.1,  
                                                                    callbacks=callbacks_list_NN, verbose=2, epochs=5000, batch_size=batchSize)
...
test_nn = pd.DataFrame({'F1': [0], 'F2': [1],'F3': [0], 'F4': [0],'F5': [0],
                 'F6': [0],'F7': [0], 'F8': [0], 'F9': [0],'F10': [0], 'num_layers_N': [0.08],
                 'num_node1_N': [0.28],'num_node2_N': [0.84]}, index=['101'])
...
concat_layer_test = Concatenate()([tf.convert_to_tensor(encoder_model.predict(split_test[1])),split_test[0]])
