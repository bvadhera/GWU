...
class Agent():
  ...
  def train(self, file_target_actions,file_target_next_state_values,file_target_next_state_values2,
      ...
      for index, row in embeddings_df.iterrows():
        ...
        state[0] = row[0] 
        state[1] = row[1]
        action = agent.act(state, True)  # ===========#RAGHAV================================= Ask Dr Huber ?
      ...
      with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
          ...
          target_values1  = rewards + gammaU * ((next_state_target_value)[0]) - ((current_state_target_value)[0])
          target_values2  =  rewards + gammaU * ((next_state_target_value)[0]) 
      ...
      if self.trainstep % self.actor_update_steps == 0:
          ...
          #Comment below to not train actor
