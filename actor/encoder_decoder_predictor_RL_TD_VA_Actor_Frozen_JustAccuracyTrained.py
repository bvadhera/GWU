...
# Helper Files
import actorCriticAgent_UpdatedAccuracyModel
import helperFunctions_JustAccuracy
import randomNetworkGeneration_UpdatedAccuracyModel
import reinforcementTraining_JustAccuracyTrained 
...
randomNetworkGeneration_UpdatedAccuracyModel.removeOneHotColumns(trainProperties)
print("normalizeProp")
print(normalizeProp)
...
num_layers,num_node1,num_node2 = randomNetworkGeneration_UpdatedAccuracyModel.getRandomLegalNetwork(divideBy,maxNumOfLayers, minNumOfLayers ) # generate valid normalize network
...
agent = actorCriticAgent_UpdatedAccuracyModel.Agent()


agent = reinforcementTraining_JustAccuracyTrained.rlTraining(test_nn_rl, model, agent, divideBy, maxNumOfLayers, minNumOfLayers,current_dir)
...
randomNetworkGeneration_UpdatedAccuracyModel.generateEmbeddingsCritic(grid_DataSet, agent,current_dir)
randomNetworkGeneration_UpdatedAccuracyModel.generateEmbeddingsActor(grid_DataSet, agent,current_dir)
...
headerPolicyOutPutFile = ['encoded_x','encoded_y', 'layers', 'nnode1','nnode2','accuracy','reward','total_reward','critic_value_state','critic_value_state_action','actor_value_1','actor_value_2'] 
...
if random100:
    ...
    for index1 in range(100):
        sameStartNode = False
        encoded_x, encoded_y,n_layers,num_node1,num_node2, prev_accuracy,reward,total_reward,qValue1, qValue2, qActions_1, qActions_2 = helperFunctions_UpdatedAccuracyModel.getPolicyoutput(index1,model,agent,divideBy,maxNumOfLayers, minNumOfLayers, decoder_model,current_dir)
        ...
        state, num_layers,num_node1,num_node2, accuracy_state = randomNetworkGeneration_UpdatedAccuracyModel.getStateAndAccuracy(model,divideBy,maxNumOfLayers, minNumOfLayers,accuracy_model)
else:
    #  same as testNN
    sameStartNode = True                     
    encoded_x, encoded_y,n_layers,num_node1,num_node2, prev_accuracy,reward,total_reward,qValue1,qValue2,qActions_1, qActions_2 = helperFunctions_UpdatedAccuracyModel.getPolicyoutput(0,model,agent,divideBy,maxNumOfLayers, minNumOfLayers, decoder_model,current_dir )
    # save into a csv file
    writerPolicyOutPut.writerow({ 'encoded_x' : encoded_x, 
            'encoded_y' : encoded_y, 
            'layers' : n_layers,
