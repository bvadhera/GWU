...
def custom_loss(y_true, y_pred):
    ...
    weight = 0.1  
    loss =  K.mean(K.square( y_true[:,1:] - y_pred[:,1:]), axis=-1)  + weight*(K.square(y_true[:,:1] - y_pred[:,:1]))
    #loss =  K.mean(K.square( y_true[:,1:] - y_pred[:,1:0), axis=-1)
    # Return a function
    return loss
...
def decoder_accuracy(y_true,y_pred):
    ...
    denorm_y_true = y_true[:,1:] * devideBy 
    denorm_y_pred = y_pred[:,1:] * devideBy 
...
predict_output = Concatenate()([accuracy_layer,decoded_layer])
...
#gradients = K.gradients(accuracy_layer.output, embedding_layer.output)

#Take enbedding value of that NN and add alpa (0.01) times the gradient 
#send this through the decoder and get the value of network and repeat until i reach the maximum.(10)
...
print ('y_pred.shape:',predictions.shape)   # shape 
...
mse_test = mean_sqe_pred(y_test_array, predictions)
