...
class Agent():
  ...
  def train(self, file_target_actions,file_target_next_state_values,file_target_next_state_values2,
      ...
      with tf.GradientTape() as tape1, tf.GradientTape() as tape2:
          ...
          target_actions = self.actor_target(states)
          if True : #Print Statements
            print('############################### target_actions from actor_target ########################################')
            print(tf.keras.backend.get_value(next_states))
          ...
          target_values1  = rewards + self.gamma * ((next_state_target_value)[0]) - ((current_state_target_value)[0])
          target_values2  =  rewards + self.gamma * ((next_state_target_value)[0]) 
          
          #Both target values should not go above -1 +1. -1 is not an issue
          ...
          boosting1 = 2.5  #Huber not 50 anymore
          boosting2 = 1
          critic_loss11 = tf.keras.losses.MSE((critic_value[1]) ,(boosting1*(target_values1)) )
          critic_loss12 = boosting2 * (tf.keras.losses.MSE((critic_value[0]) , (target_values2)))
          critic_loss1 = critic_loss11 + critic_loss12
          ...
          critic_loss21 = tf.keras.losses.MSE((critic_value2[0]) ,(boosting1*(target_values1)) )
          critic_loss22 = boosting2 * (tf.keras.losses.MSE((current_state_target_value[1]) , (target_values2)))
          critic_loss2 = critic_loss21 + critic_loss22
          if True : #Print Statements
              print('############################ critic_loss2 ########################################')
